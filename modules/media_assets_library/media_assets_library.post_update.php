 ...
 */
function media_assets_library_post_update_change_public_files_to_private() {
  ...
  $priv_path = Drupal::service('file_system')->realpath('private://');
  foreach ($result as $row) {
    ...
    if (!is_dir($priv_path . '/' . $folder) && !mkdir($concurrentDirectory = $priv_path . '/' . $folder, 0775, TRUE) && !is_dir($concurrentDirectory)) {
      throw new \RuntimeException(sprintf('Directory "%s" was not created', $concurrentDirectory));
    }
    // Move the file to the private folder.
    $new_uri = file_unmanaged_move('public://' . $uri, 'private://' . $uri, FILE_EXISTS_REPLACE);
    if ($new_uri !== NULL) {
